...
async function apiGet(action, params = {}) {
  ...
  try {
    ...
    throw new Error('Le serveur met trop de temps à répondre (surcharge probable). Réessaie dans un instant.');
  } finally {
    clearTimeout(delai);
  }
  ...
  try {
    ...
    throw new Error('Réponse serveur invalide (peut-être surchargé ou en erreur), réessaie.');
  }
}
...
async function init() {
  // Chaque attachement est défensif (?.) : si un ID a changé ou
  // manque (décalage entre versions de index.html/script.js repoussées
  // séparément), on l'ignore au lieu de faire planter TOUT init() —
  // sinon un seul ID manquant laissait l'écran totalement noir, sans
  // même l'écran de connexion, pour tout le monde.
  try {
    ...
    document.querySelectorAll('.sous-onglet-pronos').forEach(b => b.addEventListener('click', () => {
      ...
      const cartePronos = document.getElementById('carte-pronos');
      const carteSpeciaux = document.getElementById('carte-speciaux');
      if (cartePronos) cartePronos.style.display = modeProno === 'speciaux' ? 'none' : 'block';
      if (carteSpeciaux) carteSpeciaux.style.display = modeProno === 'speciaux' ? 'block' : 'none';
    ...
    document.querySelectorAll('.sous-onglet-classement').forEach(b => b.addEventListener('click', () => {
      ...
      const carteGeneral = document.getElementById('carte-classement-table');
      const carteHR = document.getElementById('carte-classement-hr');
      if (carteGeneral) carteGeneral.style.display = mode === 'general' ? 'block' : 'none';
      if (carteHR) carteHR.style.display = mode === 'hr' ? 'block' : 'none';
      if (mode === 'general') chargerClassement();
      else if (mode === 'hr') chargerClassementHR();
    }));
  } catch (err) {
    console.error('Erreur lors de l\'initialisation des boutons :', err);
  }
  ...
  // Pas de "journee" fixée ici : on laisse le serveur calculer la
  // journée en cours/à venir (journeeActuelle_), pour ne pas toujours
  // atterrir sur la journée 1 si elle est déjà terminée.
  try {
    const token = getToken();
    const reponse = await apiGet('demarrage', { token });
    ...
    if (reponse.moi) {
      if (reponse.journeeCourante) journeeCourante = Number(reponse.journeeCourante);
      matchsPreCharges_ = Array.isArray(reponse.matchs) ? reponse.matchs : null;
      matchsPreChargesJournee_ = journeeCourante;
    ...
    if (erreur) {
      ...
      message.textContent = err.message || 'Connexion au serveur impossible.';
      const btnReessayer = document.createElement('button');
      btnReessayer.textContent = 'Réessayer';
      btnReessayer.className = 'lien';
      btnReessayer.style.marginLeft = '8px';
      btnReessayer.addEventListener('click', () => location.reload());
      erreur.appendChild(message);
      erreur.appendChild(btnReessayer);
    }
  }
  const vueConnexion = document.getElementById('vue-connexion');
  if (vueConnexion) vueConnexion.style.display = 'flex';
}

let matchsPreCharges_ = null;
let matchsPreChargesJournee_ = null; // quelle journée matchsPreCharges_ concerne

let connexionEnCours_ = false;
async function connexion() {
  if (connexionEnCours_) return; // évite d'empiler des tentatives si le serveur est lent
  connexionEnCours_ = true;
  ...
  try {
    ...
    // Repasse par démarrage (et non juste "moi") pour récupérer aussi
    // la journée en cours/à venir calculée côté serveur.
    const reponseDemarrage = await apiGet('demarrage', { token: reponse.token });
    if (reponseDemarrage.journeeCourante) journeeCourante = Number(reponseDemarrage.journeeCourante);
    matchsPreCharges_ = Array.isArray(reponseDemarrage.matchs) ? reponseDemarrage.matchs : null;
    matchsPreChargesJournee_ = journeeCourante;
    afficherApp(reponseDemarrage.moi);
  } catch (err) {
    erreur.textContent = 'Connexion au serveur impossible. Réessaie dans un instant.';
  } finally {
    connexionEnCours_ = false;
    if (btn) { btn.disabled = false; btn.textContent = texteOriginal; }
  }
...
function afficherApp(joueur) {
  ...
  // Bloc admin isolé : un ID de carte admin manquant ne doit jamais
  // empêcher le reste (matchs, classement...) de s'afficher pour un
  // joueur normal ou même pour un admin.
  try {
    if (joueur.admin) {
      const ongletAdmin = document.getElementById('onglet-admin');
      if (ongletAdmin) ongletAdmin.style.display = 'inline-block';
    ...
    console.error('Erreur bloc admin (ignorée) :', err);
  }
...
function changerOnglet(vue) {
  ...
  if (vue === 'admin') peuplerListesAdmin_();
}

// Peuple les 3 menus déroulants "joueur" de l'écran Admin (mdp, retrait,
// historique) — appelé une seule fois, à la première ouverture de
// l'onglet Admin, pas au chargement de la page pour tout le monde
// (ces menus n'existent que pour les admins, inutile de faire cet
// appel réseau supplémentaire pour chaque joueur normal).
let listesAdminPeuplees_ = false;
async function peuplerListesAdmin_() {
  if (listesAdminPeuplees_) return;
  listesAdminPeuplees_ = true;

  try {
    const joueurs = await apiGet('joueurs');
    [
      document.getElementById('admin-mdp-joueur'),
      document.getElementById('admin-joueur-retrait'),
      document.getElementById('admin-historique-joueur'),
    ].forEach(select => {
      if (!select) return;
      select.innerHTML = '';
      (joueurs || []).forEach(j => {
    ...
    listesAdminPeuplees_ = false; // on retentera à la prochaine ouverture
  }
}
